...
public class HxVolumetricLight : MonoBehaviour
{
    ...
    {
        Vector4 BasedColor = (CustomColor ? Color : LightColor()) * (CustomIntensity ? Intensity : LightIntensity()) * CalcLightInstensityDistance(distance);
        ...
        if (ShaderModel4())
        {
            ...
            for (int i = 0; i < HxVolumetricCamera.ActiveVolumes.Count; i++)
            {
                if (HxVolumetricCamera.ActiveVolumes[i].enabled && ((HxVolumetricCamera.ActiveVolumes[i].BlendMode != HxDensityVolume.DensityBlendMode.Add || HxVolumetricCamera.ActiveVolumes[i].BlendMode != HxDensityVolume.DensityBlendMode.Sub) && HxVolumetricCamera.ActiveVolumes[i].Density != 0))
                {
                    ...
                    VolumeSettingsArrays[c] = new Vector2(HxVolumetricCamera.ActiveVolumes[i].Density, (int)HxVolumetricCamera.ActiveVolumes[i].BlendMode);
                    c++;
        ...
        else
        {
            ...
            for (int i = 0; i < HxVolumetricCamera.ActiveVolumes.Count; i++)
            {
                if (HxVolumetricCamera.ActiveVolumes[i].enabled && ((HxVolumetricCamera.ActiveVolumes[i].BlendMode != HxDensityVolume.DensityBlendMode.Add || HxVolumetricCamera.ActiveVolumes[i].BlendMode != HxDensityVolume.DensityBlendMode.Sub) && HxVolumetricCamera.ActiveVolumes[i].Density != 0))
                {
                    ...
                    VolumeSettingsArraysOld[c] = new Vector2(HxVolumetricCamera.ActiveVolumes[i].Density, (int)HxVolumetricCamera.ActiveVolumes[i].BlendMode);
                    c++;
    ...
    void LoadVolumeDataBounds()
    {
        ...
        for (int i = 0; i < HxVolumetricCamera.ActiveVolumes.Count; i++)
        {
            if (HxVolumetricCamera.ActiveVolumes[i].Density != 0 && BoundsIntersect(HxVolumetricCamera.ActiveVolumes[i]))
            {
                VolumeMatrixArrays[c] = HxVolumetricCamera.ActiveVolumes[i].ToLocalSpace;
                VolumeSettingsArrays[c] = new Vector2(HxVolumetricCamera.ActiveVolumes[i].Density, (int)HxVolumetricCamera.ActiveVolumes[i].BlendMode);
                c++;
        ...
        if (c < 25)
        {
            VolumeSettingsArrays[c] = new Vector2(0, 0);
        }
    }
