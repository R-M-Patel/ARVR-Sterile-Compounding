...
#include "UnityCG.cginc"
	...
	void TemporalSample(float depth, float2 uv, inout float4 outColor)
	{
		...
		else
		{
			...
			//if (length(lastUV - uv) < _CameraDepthTexture_TexelSize.x * 2) { lastUV = uv; }
			//lastUV.y -= _CameraDepthTexture_TexelSize.y * 0.5f;
#endif
...
#ifdef UNITY_SINGLE_PASS_STEREO	
		//outColor = float4(Left, !Left, 0, 0);
#endif

		//outColor = float4(uv2, 0, 0);
	}
