...
#include "UnityCG.cginc"
	...
	float4 GetNearestDepthSample(float LZ, float2 uv, out float blur)
	{
		...
		float Z00 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(mid.xy, 0, 0)).rg)); float az = Z00; float az2 = Z00;
		UpdateNearestSample2(MinDist, NearestUV, MinDist2, NearestUV2, az, az2, Z00, mid, LZ);

		float2 UV10 = float2(mid.x - offsetTexel.x, mid.y);
		float Z10 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV10, 0, 0)).rg) );
		...
		float Z01 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV01, 0, 0)).rg) );
		UpdateNearestSample2(MinDist, NearestUV, MinDist2, NearestUV2, az, az2, Z01, UV01, LZ);

		float2 UV11 = mid - offsetTexel;
		float Z11 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV11, 0, 0)).rg) );
		...
		else
		{
			...
			float Z20 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV20, 0, 0)).rg));
			...
			float Z21 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV21, 0, 0)).rg));
			...
			float ZN1 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UVN1, 0, 0)).rg));
			...
			float ZN0 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UVN0, 0, 0)).rg));
			...
			float Z02 = (DecodeFloatRG(tex2Dlod(VolumetricDepth, float4(UV02, 0, 0)).rg));
			...
			if (MinDist2 > 99999) { NearestUV2 = NearestUV; az2 = az;
			}
	...
	void TemporalSample(float depth, float2 uv,float2 uv2, inout float4 outColor, inout int blur)
	{
		...
		cameraRay = (cameraRay / cameraRay.w);
		cameraRay.xyz = cameraRay.xyz / cameraRay.z;
		float3 wpos = mul(hxCameraToWorld, float4(-cameraRay.xyz * LinearEyeDepth(depth), 1)).xyz;
		...
		else
		{
			...
			float diff = saturate( 1.0f / max(abs(lum0 - lum1),0.0001f) * min(lum0,lum1));				
			float feedback = lerp(hxTemporalSettings.x, hxTemporalSettings.y, diff);
	...
	void fragTemp(v2cf input, out fixed4 outColor : SV_Target, out float outDepth : SV_Depth)
	{
		...
		float blur = 1;
		outColor = GetNearestDepthSample(Linear01Depth(centerDepth),input.uv.xy / input.uv.w, blur);
		TemporalSample(centerDepth, input.uv / input.uv.w, input.uv2, outColor, blur);
		outDepth = blur;
	}
	...
	bvr frag2Temp(v2cf input)
	{
		...
		float4 outcome = GetNearestDepthSample(Linear01Depth(centerDepth), input.uv.xy, blur);
		TemporalSample(centerDepth, input.uv, input.uv2, outcome, blur);
		outbvr.col0 = outcome;
		outbvr.col1 = outcome;
		outbvr.depth0 = blur;
		return outbvr;
